...
router.post('/migrate', upload.fields([
  ...
  try {
    ...
    const sheets = getSheetsClient();
    if (!dryRun && (!sheets || !CONFIG.DB_SHEET_ID)) return res.status(503).json({ error: 'Sheets not configured' });
    ...
    if (req.files.hanosCsv && req.files.hanosCsv[0]) {
      ...
      const catIdx = col('categorie');
      const subCatIdx = col('subcategorie');
      const totalOrderedIdx = col('totaal_aantal_besteld');
    ...
    if (dryRun) {
      // Return stats + sample of first 20 items
...
router.post('/stock', async (req, res) => {
  ...
  try {
    await withWriteLock(async () => {
      ...
      ing.stock[location] = { amount: parseFloat(amount) || 0, date: new Date().toISOString().slice(0, 10) };
...
router.post('/stock/bulk', async (req, res) => {
  ...
  try {
    await withWriteLock(async () => {
      ...
      let updated = 0;
      updates.forEach(u => {
        const ing = byId[u.ingredientId];
        if (!ing) return;
        if (!ing.stock) ing.stock = {};
        ing.stock[u.location] = { amount: parseFloat(u.amount) || 0, date: new Date().toISOString().slice(0, 10) };
        updated++;
      });
      await writeTab(sheets, CONFIG.DB_SHEET_ID, 'ingredients', INGREDIENT_HEADERS, all.map(ingredientToRow));
    });

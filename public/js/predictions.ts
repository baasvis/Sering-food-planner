...
export function parseCSV(text: any) {
  const lines = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
  if (lines.length < 2) return [];

  function splitLine(line: any) {
    const fields = [];
    let current = '';
    let inQuotes = false;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    const row = {};
    headers.forEach((h: any, j: any) => { row[h] = (vals[j] || '').trim(); });
    rows.push(row);
  }
  return rows;
}
...
export function emptyAggregated() {
  return {
    west: { lunch: {}, dinner: {}, staff_lunch: {}, staff_dinner: {} },
    centraal: { lunch: {}, dinner: {}, staff_lunch: {}, staff_dinner: {} }
  };
}
export const AGG_MEALS = ['lunch', 'dinner', 'staff_lunch', 'staff_dinner'];

export function categorizeUploadedFiles(fileContents: any, existingDeviceMap: any) {
...
export function buildFlowDistribution(timeEvents: any) {
  ...
  for (const loc of Object.keys(raw)) {
    ...
    for (const meal of Object.keys(raw[loc])) {
      ...
      for (const dow of Object.keys(raw[loc][meal])) {
        ...
        const total = Object.values(buckets).reduce((s: any, v: any) => s + v, 0);
        if (total === 0) continue;
        dist[loc][meal][dow] = {};
...
export function averageLayers(layers: any) {
  ...
  for (const loc of ['west', 'centraal']) {
    for (const meal of AGG_MEALS) {
      ...
      for (const [date, values] of Object.entries(dateValues)) {
        result[loc][meal][date] = Math.round(values.reduce((a: any, b: any) => a + b, 0) / values.length);
      }
    }
  }
  return result;
...
export function detectFormat(text: any) {
...
export function parseSemicolonCSV(text: any) {
  const lines = text.replace(/\r\n/g, '\n').replace(/\r/g, '\n').split('\n');
  if (lines.length < 2) return [];
...
export function mergeAggregated(target: any, source: any) {
  for (const loc of ['west', 'centraal']) {
...
export function categorizeProfitCenterData(rows: any, date: any) {
...
export function categorizeLightspeedData(rows: any) {
  const aggregated = emptyAggregated();
  const timeEvents = [];
  let totalMealRows = 0;
...
export function parseLightspeedMinuteOfDay(dateStr: any) {
...
export function parseLightspeedHour(dateStr: any) {
...
export function parseLightspeedDate(dateStr: any) {
  if (!dateStr) return null;
...
export function categorizeTebiData(allRows: any, existingDeviceMap: any) {
  const deviceMap = { ...(existingDeviceMap || {}) };
...
export function extractMinuteOfDayFromTebiRow(row: any) {
...
export function extractHourFromTebiRow(row: any) {
  const invoiceId = row['Invoice ID'] || '';
  const parts = invoiceId.split('|');
  if (parts.length >= 2) {
...
//                                               ^^^^^^^^^^
export function extractDeviceId(invoiceId: any) {
...
export function predictGuests(history: any) {
  const predictions = {};
  ...
  for (const loc of ['west', 'centraal']) {
    ...
    for (const meal of ['lunch', 'dinner', 'staff_lunch', 'staff_dinner']) {
      ...
      const entries = Object.entries(dateMap)
        .map(([date, count]: any) => ({ date, count, dow: getDayOfWeek(date) }))
      ...
      for (const day of DAY_NAMES) {
        ...
        if (winsorized.length >= 4) {
          ...
          const avgFirst = firstHalf.reduce((a: any, b: any) => a + b, 0) / firstHalf.length;
          const avgSecond = secondHalf.reduce((a: any, b: any) => a + b, 0) / secondHalf.length;
          if (avgFirst > 0) {
...
export function winsorize(values: any) {
...
export function percentile(sorted: any, p: any) {
...
export function getDayOfWeek(dateStr: any) {
...
export function getVisibleDays(offset: any) {

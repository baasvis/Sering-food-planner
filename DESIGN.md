...
| Drink | id, name, mode (catalogue/recipe), category, subtype, abv, btwRate, status, archived, sellable, supplier, orderUnit, orderUnitMl, deposit, costPrice, formats (JSON), locations (JSON: par + per-location active + home storage area), info (JSON), tebiProductNames, recipe fields (serveVolumeMl, glass, characteristics, garnish, prepSteps JSON, batch JSON, prepTime JSON, shelfLifeDays), costPerServe, suggestedPrice, photoUrl | Drink / drinks |

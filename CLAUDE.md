...
- A service entry may carry `pinned: true` (📌 on the planner chip, `toggleServicePin`): Fix My Menu's `stripFutureServices` keeps pinned assignments and `findStalePlaceholders` won't retire a placeholder carrying a pinned upcoming service, so the algorithm plans around pins instead of redistributing them. Lives inside the `services` JSON — no schema migration.
- A batch an **active** catering points at (`catering.dishes[].dishId`, gated by core's `cateringActive`) is spared from Fix My Menu's auto-retire cleanups (`findOrphanPlaceholders`/`findSpentBatches`/`findStalePlaceholders` via `cateringReferencedBatchIds` in menu-fixer.ts). A catering pick is a `catering.dishes` ref, NOT a Batch service, so `stripFutureServices` empties a catering-held placeholder's services and it would otherwise look like an orphan and be dropped from the event. Delivered (past) caterings pull no demand, so their dead placeholders are still cleaned up.

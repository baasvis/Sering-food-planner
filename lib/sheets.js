...
const INGREDIENT_HEADERS = [
  'id','name','supplier_name','types','category','unit','supplier',
  'order_code','order_unit','order_unit_standard','order_price',
  'order_amount_grams','price_level','price_per_100g','price_history',
  'price_alert','storage_locations','stock','nutrition',
  'allergens','notes','active'
];
...
function rowToIngredient(row) {
  const orderPrice = row.order_price ? parseFloat(row.order_price) : null;
  const orderAmountGrams = parseFloat(row.order_amount_grams) || 0;
  return {
    id: row.id,
    name: row.name || '',
    ...
    pricePer100g: parseFloat(row.price_per_100g) || 0,
    priceHistory: safeJsonParse(row.price_history, []),
    priceAlert: row.price_alert === 'true',
    storageLocations: safeJsonParse(row.storage_locations, {}),
    stock: safeJsonParse(row.stock, {}),
    nutrition: safeJsonParse(row.nutrition, {}),
...
function ingredientToRow(ing) {
  return [
    ing.id, ing.name || '', ing.supplierName || '',
    JSON.stringify(ing.types || []),
    ing.category || '', ing.unit || 'Grams', ing.supplier || '',
    ing.orderCode || '', ing.orderUnit || '', ing.orderUnitStandard || '',
    ing.orderPrice != null ? ing.orderPrice : '',
    ing.orderAmountGrams || 0,
    ing.priceLevel || '',
    ing.pricePer100g || 0,
    JSON.stringify(ing.priceHistory || []),
    ing.priceAlert ? 'true' : 'false',
    JSON.stringify(ing.storageLocations || {}),
    JSON.stringify(ing.stock || {}),
    JSON.stringify(ing.nutrition || {}),
    ing.allergens || '', ing.notes || '',
    ing.active !== false ? 'true' : 'false',
  ];
}
...
async function dbWriteDishes(dishes) {
  const sheets = getSheetsClient();
  if (!sheets || !CONFIG.DB_SHEET_ID) return;
  await ensureTabsExist(sheets, CONFIG.DB_SHEET_ID, ['dishes', 'services']);
  await writeTab(sheets, CONFIG.DB_SHEET_ID, 'dishes', DISH_HEADERS, dishes.map(dishToRow));
  const serviceRows = [];
  dishes.forEach(dish => {
    (dish.services || []).forEach(svc => {
...
async function dbWriteGuests(guests) {
  const sheets = getSheetsClient();
  if (!sheets || !CONFIG.DB_SHEET_ID) return;
  await ensureTabsExist(sheets, CONFIG.DB_SHEET_ID, ['guests']);
  const guestRows = [];
  Object.entries(guests).forEach(([loc, days]) => {
    Object.entries(days).forEach(([day, meals]) => {
      guestRows.push([loc, day, meals.lunch || 0, meals.dinner || 0]);
...
async function dbWriteCaterings(caterings) {
  const sheets = getSheetsClient();
  if (!sheets || !CONFIG.DB_SHEET_ID) return;
  await ensureTabsExist(sheets, CONFIG.DB_SHEET_ID, ['caterings']);
  await writeTab(sheets, CONFIG.DB_SHEET_ID, 'caterings', CATERING_HEADERS, caterings.map(cateringToRow));
}
...
async function dbWriteTransportItems(items) {
  const sheets = getSheetsClient();
  if (!sheets || !CONFIG.DB_SHEET_ID) return;
  await ensureTabsExist(sheets, CONFIG.DB_SHEET_ID, ['transport_items']);
  const TRANSPORT_ITEM_HEADERS = ['id', 'text'];
  await writeTab(sheets, CONFIG.DB_SHEET_ID, 'transport_items', TRANSPORT_ITEM_HEADERS, items.map(i => [i.id, i.text]));
}

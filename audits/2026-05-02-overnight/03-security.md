...
**RESOLVED on 2026-05-03 (branch `claude/musing-robinson-acfca7`)**: added `VALID_ID_PATTERN = /^[a-zA-Z0-9_-]{1,200}$/` and applied it across `validateBatch` (id + parentId + recipeId), `validateCatering` (id + dishes[].dishId), `validateTransportItem` (id), `validateRecipe` (id + ingredients[].id + ingredients[].ingredientId — id was unchecked before), `validateIdList`, plus the bulk and per-id ingredient POST routes via the new exported `checkId` helper. PATCH /recipes/:id now always validates the body. Verified all 1256 prod IDs and 1251 staging IDs match the regex (no compat hit). 13 new integration tests in `test/api.test.ts`. End-to-end preview check confirms all five S2 vectors return 400 against the live staging server. Deferred: defense-in-depth refactor of `onclick=""` interpolations to delegated handlers (so a future renderer that drops `esc()` can't reopen the vector) — keeping that for a separate CSP-related PR.
- **Severity**: High

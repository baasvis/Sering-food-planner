...
**PARTIALLY RESOLVED on 2026-05-04 (branch `claude/t19-bulk-recalc-acfca7`)**: added `recalcAllRecipeCosts` to `lib/db.ts` (one read of all recipes + one read of all ingredient prices, then per-row updates). Bulk POST `/api/ingredients` now awaits it and surfaces a `recipeCostsUpdated` count in the response so a future regression that drops the recalc fails-loud. Single regression test in `test/api.test.ts`.

**BUT**: while implementing this I discovered a more serious data-corruption bug — the bulk POST's `deleteMany + createMany` shape interacts with the `ON DELETE SET NULL` FK on `recipe_ingredients.ingredient_id` and silently wipes every recipe→ingredient link on every supplier import (staging is currently 100% NULL — 618/618 rows). Until that root cause (now filed as **T19a in 99-followups.md**) is fixed via a raw `INSERT … ON CONFLICT DO UPDATE`, the recalc trigger here can only produce null costs because it sees no linked ingredients. The recalc trigger is still useful — once T19a lands, T19 starts producing real numbers immediately with no further changes.
- **Severity**: Medium
